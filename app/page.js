...
function AbaMetricas({ metricas, onSalvar, onDeletar, onEditar }) {
  ...
  const porPlat = PLATAFORMAS.map(p => ({ nome: p, count: metricas.filter(m => m.plataforma === p && m.noAr === 'sim').length }))
  ...
  return (
    <div style={{ padding:'1.5rem', overflowY:'auto', flex:1 }}>
      ...
      <div style={{ background:CARD, borderRadius:10, padding:'16px' }}>
        ...
        {filtradas.length === 0 && <p style={{ color:SUBTEXTO, fontSize:13, textAlign:'center', padding:'2rem 0' }}>Nenhum resultado cadastrado.</p>}
