...
function AbaMetricas({ metricas, onSalvar, onDeletar, onEditar, pautas }) {
  ...
  function importarPauta(p) {
    ...
    setModoImportar(false); setMostrarForm(true)
  }
  ...
  async function salvar() {
    ...
    setForm(VAZIO_METRICA); setMostrarForm(false)
  }
  ...
  return (
    <div style={{ padding:'1.5rem', overflowY:'auto', flex:1 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12, marginBottom:20 }}>
        {[{label:'Total de pautas',value:total,cor:TEXTO},{label:'Entraram no ar',value:noAr,cor:VERDE},{label:'Aproveitamento',value:`${aproveitamento}%`,cor:corAprov},{label:'Total de views',value:formatViews(totalViews),cor:AMARELO}].map(({label,value,cor}) => (
          ...
          {PLATAFORMAS.map((p, i) => {
            ...
            return (
              <div key={p} style={{ marginBottom:10 }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:TEXTO, marginBottom:4 }}><span>{p}</span><span style={{ color:SUBTEXTO }}>{count} ({pct}%)</span></div>
                <div style={{ background:'#2A2A2A', borderRadius:4, height:8 }}><div style={{ background:CORES_PLAT[i], borderRadius:4, height:8, width:`${pct}%`, transition:'width 0.3s' }} /></div>
              </div>
            )
          })}
        </div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:120 }}>
            <div style={{ position:'relative', width:120, height:120 }}>
              <svg viewBox="0 0 120 120" style={{ transform:'rotate(-90deg)' }}>
                ...
                <circle cx="60" cy="60" r="50" fill="none" stroke={corAprov} strokeWidth="12" strokeDasharray={`${aproveitamento * 3.14} 314`} strokeLinecap="round" />
              </svg>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, flexWrap:'wrap', gap:10 }}>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {['Todas',...PLATAFORMAS].map(p => (
              <button key={p} onClick={() => setFiltroPlat(p)} style={{ padding:'5px 12px', border:'none', borderRadius:20, cursor:'pointer', fontSize:12, fontWeight:600, background:filtroPlat===p?AMARELO:'#2A2A2A', color:filtroPlat===p?'#000':SUBTEXTO }}>{p}</button>
            ))}
          </div>
            ...
            <div style={{ maxHeight:240, overflowY:'auto', display:'flex', flexDirection:'column', gap:6 }}>
              ...
              {pautasFiltradas.map(p => (
                <div key={p.id} onClick={() => importarPauta(p)} style={{ background:CARD, border:`1px solid ${BORDA}`, borderRadius:8, padding:'10px 14px', cursor:'pointer' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor=AMARELO} onMouseLeave={e => e.currentTarget.style.borderColor=BORDA}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                {PLATAFORMAS.map((p, i) => {
                  ...
                  return <button key={p} type="button" onClick={() => togglePlat(p)} style={{ padding:'6px 14px', borderRadius:20, cursor:'pointer', fontSize:12, fontWeight:700, border:sel?'none':`1px solid ${BORDA}`, background:sel?CORES_PLAT[i]:'#2A2A2A', color:sel?(p==='Instagram'?'#000':'#fff'):SUBTEXTO }}>{p}</button>
                })}
              </div>
            ...
            {(form.plataformas||[]).length > 0 && (
              <div style={{ background:'#1a1a1a', border:`1px solid #333`, borderRadius:10, padding:'12px 14px', marginBottom:14 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px, 1fr))', gap:10 }}>
                  {(form.plataformas||[]).includes('Instagram') && <div><label style={{ fontSize:11, color:AMARELO, fontWeight:700, textTransform:'uppercase' }}>Posts Instagram</label><input type="number" placeholder="Qtd. posts" value={form.postsInsta} onChange={e => setForm({...form,postsInsta:e.target.value})} style={inp} /></div>}
                  {(form.plataformas||[]).includes('YouTube') && <div><label style={{ fontSize:11, color:'#E1306C', fontWeight:700, textTransform:'uppercase' }}>Entradas YouTube</label><input type="number" placeholder="Qtd. entradas" value={form.entradasYT} onChange={e => setForm({...form,entradasYT:e.target.value})} style={inp} /></div>}
                  {(form.plataformas||[]).includes('Programas') && <div><label style={{ fontSize:11, color:'#9146FF', fontWeight:700, textTransform:'uppercase' }}>Entradas Programas</label><input type="number" placeholder="Qtd. entradas" value={form.entradasProg} onChange={e => setForm({...form,entradasProg:e.target.value})} style={inp} /></div>}
                  {(form.plataformas||[]).includes('Transmissões') && <div><label style={{ fontSize:11, color:'#00b4d8', fontWeight:700, textTransform:'uppercase' }}>Entradas Transmissões</label><input type="number" placeholder="Qtd. entradas" value={form.entradasTrans} onChange={e => setForm({...form,entradasTrans:e.target.value})} style={inp} /></div>}
                </div>
              </div>
            )}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
              <div><label style={{ fontSize:11, color:SUBTEXTO, fontWeight:700, textTransform:'uppercase' }}>Entrou no ar?</label><select value={form.noAr} onChange={e => setForm({...form,noAr:e.target.value})} style={{...inp,cursor:'pointer'}}><option value="sim">Sim</option><option value="nao">Não</option></select></div>
              <div><label style={{ fontSize:11, color:SUBTEXTO, fontWeight:700, textTransform:'uppercase' }}>Total de visualizações</label><input type="number" placeholder="Ex: 50000" value={form.views} onChange={e => setForm({...form,views:e.target.value})} style={inp} /></div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
        ...
        {filtradas.length > 0 && (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead><tr>{['Data','Pauta','Repórter','Plataformas','No ar','Views',''].map(h => <th key={h} style={{ textAlign:'left', color:SUBTEXTO, fontSize:11, fontWeight:600, textTransform:'uppercase', padding:'6px 8px', borderBottom:`0.5px solid ${BORDA}` }}>{h}</th>)}</tr></thead>
              <tbody>
                  ...
                  <tr key={m.id}>
                    ...
                    <td style={{ padding:'9px 8px', borderBottom:`0.5px solid ${BORDA}` }}>
                      <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                        {getPlats(m).length>0 ? getPlats(m).map(p => { const i=PLATAFORMAS.indexOf(p); return <span key={p} style={{ background:i>=0?CORES_PLAT[i]:'#2A2A2A', color:p==='Instagram'?'#000':'#fff', borderRadius:20, padding:'2px 10px', fontSize:11, fontWeight:600 }}>{p}</span> }) : <span style={{ color:SUBTEXTO }}>—</span>}
                      </div>
                    </td>
                    <td style={{ padding:'9px 8px', borderBottom:`0.5px solid ${BORDA}` }}>
                      {m.noAr==='sim' ? <span style={{ background:'#0a2a0a', color:VERDE, border:`0.5px solid ${VERDE}`, borderRadius:20, padding:'2px 10px', fontSize:11, fontWeight:700 }}>✓ Sim</span> : <span style={{ background:'#2a0a0a', color:'#ff4444', border:'0.5px solid #ff4444', borderRadius:20, padding:'2px 10px', fontSize:11, fontWeight:700 }}>✗ Não</span>}
                    </td>
                    <td style={{ padding:'9px 8px', borderBottom:`0.5px solid ${BORDA}` }}>
...
export default function Home() {
  ...
  useEffect(() => {
    if (!hasSidebar || datas.length === 0) return
  ...
  async function carregarTudo() {
    ...
    const [rP,rR,rPrev,rC,rPed,rM] = await Promise.all([
      fetch('/api/pautas').then(r=>r.json()),fetch('/api/relatorios').then(r=>r.json()),
      fetch('/api/previsoes').then(r=>r.json()),fetch('/api/contatos').then(r=>r.json()),
      fetch('/api/pedidos').then(r=>r.json()),fetch('/api/metricas').then(r=>r.json()),
    ])
  ...
  async function gerarRelatorio() {
    setGerando(true); setRelatorioGerado('')
    try {
    ...
    } catch(e) { setRelatorioGerado('<p style="color:red">Erro ao conectar. Tente novamente.</p>') }
    setGerando(false)
  }
  ...
  function copiarRelatorio() {
    ...
    navigator.clipboard.write([clipItem]).then(() => { setCopiado(true); setTimeout(()=>setCopiado(false),2500) }).catch(() => {
      const el = document.createElement('div'); el.innerHTML = relatorioGerado; document.body.appendChild(el)
      ...
      document.execCommand('copy'); window.getSelection().removeAllRanges(); document.body.removeChild(el)
  ...
  function baixarPDF() {
    ...
    janela.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Pautas CazéTV — ${dataLabel}</title><style>body{font-family:Arial,sans-serif;font-size:13px;color:#111;padding:32px 40px;max-width:680px;margin:0 auto}@media print{body{padding:0}@page{margin:20mm}}</style></head><body>${relatorioGerado}</body></html>`)
    janela.document.close(); janela.focus(); setTimeout(()=>janela.print(),400)
  }
  ...
  async function handlePdf(e) {
    ...
    const data=await res.json()
    setFormPauta(f=>({...f,pdfUrl:data.url})); setUploadando(false)
  }
  ...
  const abas = [
    {id:'pautas',label:'📋 Pautas',cor:AMARELO},{id:'relatorios',label:'📝 Relatórios',cor:AMARELO},
    {id:'previsoes',label:'🔭 Previsões',cor:AMARELO},{id:'pedidos',label:'📥 Pedidos',cor:LARANJA},
    {id:'contatos',label:'📞 Contatos',cor:AMARELO},{id:'busca',label:'🔍 Busca',cor:AMARELO},
    {id:'calendario',label:'📅 Calendário',cor:AMARELO},{id:'metricas',label:'📊 Métricas',cor:AMARELO},
  ]
  ...
  function CardPauta({p}) {
    ...
    return (
      <div style={{background:CARD,border:`1px solid ${BORDA}`,borderRadius:12,padding:'1rem 1.2rem'}} onMouseEnter={e=>e.currentTarget.style.borderColor=AMARELO} onMouseLeave={e=>e.currentTarget.style.borderColor=BORDA}>
    ...
    <main style={{minHeight:'100vh',background:ESCURO,color:TEXTO,fontFamily:"'Inter','Helvetica Neue',sans-serif",display:'flex',flexDirection:'column'}}>
      <header style={{background:'#000',borderBottom:`3px solid ${AMARELO}`,padding:'0.85rem 1.5rem',display:'flex',alignItems:'center',gap:14,flexShrink:0}}>
        ...
        <div style={{marginLeft:'auto'}}>
          <button onClick={()=>{setModalRelatorio(true);gerarRelatorio()}} style={{background:AMARELO,color:'#000',border:'none',borderRadius:8,padding:'8px 16px',cursor:'pointer',fontWeight:700,fontSize:13}}>📋 Relatório do dia</button>
        </div>
      </header>
      ...
      <div style={{display:'flex',gap:2,padding:'6px 10px',background:'#0A0A0A',borderBottom:`1px solid ${BORDA}`,flexShrink:0,flexWrap:'wrap'}}>
        {abas.map(a=>(
          <button key={a.id} onClick={()=>{setAba(a.id);setDataSelecionada(null);setMostrarForm(false);setBusca('');setBuscaGeral('')}} style={{padding:'7px 16px',border:'none',borderRadius:7,cursor:'pointer',fontWeight:700,fontSize:13,background:aba===a.id?a.cor:'transparent',color:aba===a.id?(a.id==='pedidos'?'#fff':'#000'):SUBTEXTO,transition:'all 0.15s'}}>{a.label}</button>
        ))}
      </div>
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden',minHeight:0}}>
        {hasSidebar&&(
          <aside style={{width:220,flexShrink:0,borderRight:`1px solid ${BORDA}`,padding:'12px 8px',overflowY:'auto',display:'flex',flexDirection:'column',gap:4,background:'#0D0D0D'}}>
            ...
            {loading&&<div style={{padding:'0 6px',display:'flex',flexDirection:'column',gap:6}}>{[1,2,3].map(i=><div key={i} style={{height:44,background:CARD,borderRadius:8,opacity:0.5}}/>)}</div>}
            {!loading&&datas.length===0&&<div style={{padding:'1.5rem 6px',textAlign:'center'}}><p style={{fontSize:12,color:SUBTEXTO,margin:0,lineHeight:1.6}}>Nenhum registro ainda.</p></div>}
            {datas.map(d=>{
              ...
              return (
                <button key={d} onClick={()=>{setDataSelecionada(d);setMostrarForm(false)}} style={{width:'100%',textAlign:'left',border:'none',borderRadius:8,padding:'9px 12px',cursor:'pointer',background:isActive?corAba:CARD,color:isActive?(aba==='pedidos'?'#fff':'#000'):TEXTO,outline:isActive?'none':`1px solid ${BORDA}`,transition:'all 0.1s'}}>
        ...
        {aba==='metricas'&&(
          <AbaMetricas metricas={metricas} pautas={pautas}
                  ...
                  {[
                    ...
                    <div key={label} style={{marginBottom:'1.5rem'}}>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                        <div style={{width:4,height:16,background:AMARELO,borderRadius:2}}/><span style={{fontSize:12,fontWeight:700,color:SUBTEXTO,textTransform:'uppercase',letterSpacing:1}}>{label}</span><span style={{fontSize:11,color:'#555',marginLeft:4}}>{items.length} resultado(s)</span>
                      </div>
