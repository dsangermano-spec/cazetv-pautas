...
export async function GET() {
  try {
    const [pautas, relatorios] = await Promise.all([
      redis.get('pautas').then(r => r || []),
      redis.get('relatorios').then(r => r || []),
    ])
    ...
    const pautasAmanha = pautas.filter(p =>
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; font-size: 14px; color: #111; padding: 32px 40px; max-width: 680px; margin: 0 auto; }
